...
{
    /* Decode minimal, case-sensitive, sans allocation */
    CLICmd_t cmd = CLI_CMD_INVALID;

    if ((buf != NULL) && (len != 0U))
    {
        /* Comparaisons directes (pas besoin de string) */
...
void CliUart_OnRxChar(uint8_t c)
{
	...
	{
		/* Ne valide une ligne que si on a au moins 1 char */
		if (rx_len != 0U)
		{
			end_line = 1U;
		}
...
CLIStatus_t CliUart_Process(CLICmd_t *out_cmd)
{
    ...
    else
    {
        *out_cmd = CLI_CMD_INVALID;
        ...
        {
        	/* variableScope */
            uint32_t len_copy = 0U;
            uint8_t line_copy[CLI_UART_MAX_LINE_LEN];
            uint32_t i;

            /* Clone de len car ISR peut modifier rx_len */
            len_copy = rx_len;

        	/* Clone du buffer car ISR peut modifier */
            ...
            cmd = CliUart_DecodeLine(&line_copy[0], len_copy);
            *out_cmd = cmd;
        }
    }
...
CLIStatus_t CliUart_TxChar(uint8_t c)
{
    ...
    hal_status = HAL_UART_Transmit(&huart3, &c,
                                   (uint16_t)CLI_UART_TX_LEN_BYTES,
                                   (uint32_t)CLI_UART_TX_TIMEOUT_MS);

    if (hal_status != HAL_OK)
    {
        status = CLI_STATUS_ERR_PARAM; /* ou CLI_STATUS_ERR_UART */
    }
